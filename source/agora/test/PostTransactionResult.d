...
unittest
{
    ...
    auto tx = new TxBuilder(input_txs[0], 0).feeRate(Amount(10_000))
    ...
    tx = new TxBuilder(input_txs[0], 0).feeRate(Amount(10_000))
        .draw(Amount(100_000), [output_addr])
        .sign();
    ...
    tx = new TxBuilder(input_txs[0], 1).feeRate(Amount(0))
        .draw(Amount(100_000), [output_addr])
        .sign();
    result = nodes[0].postTransaction(tx);
    assert(result.status == TransactionResult.status.Rejected);
