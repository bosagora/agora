...
public class FlashValidator : Validator, FlashValidatorAPI
{
    ...
    ***************************************************************************/
    ...
    public this (const Config config)
    {
        ...
        immutable kp = Pair.fromScalar(secretKeyToCurveScalar(
            this.config.validator.key_pair.secret));
    ...
    private void runAliceTasks ()
    {
        ...
        const alice_pair = Pair.fromScalar(secretKeyToCurveScalar(WK.Keys.NODE2.secret));
        const bob_pair = Pair.fromScalar(secretKeyToCurveScalar(WK.Keys.NODE3.secret));
        const charlie_pair = Pair.fromScalar(secretKeyToCurveScalar(WK.Keys.NODE4.secret));
