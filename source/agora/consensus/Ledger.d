...
public class Ledger
{
    ...
    /// Holds the Coinbase txs on payout blocks (it is array although has only one)
    private Transaction coinbase_tx;
    ...
    ***************************************************************************/
    ...
    private void addValidatedBlock (in Block block) @safe
    {
        ...
        {
            this.fee_man.clearAccumulatedFees(); // Clear for next payout cycle
        }
        ...
        // reset the Coinbase tx is we just paid out
        if (block.header.height % this.params.PayoutPeriod == 0)
            this.coinbase_tx = Transaction.init;
    ...
    ***************************************************************************/
        ...
        nothrow @safe
    {
        ...
        auto payouts = this.fee_man.getAccumulatedFees(height);
    ...
    ***************************************************************************/
        ...
        @safe nothrow
    {
        ...
        // This is payout block
        if (expect_height % this.params.PayoutPeriod == 0)
        {
            this.coinbase_tx = this.getCoinbaseTX(expect_height);
            assert(this.coinbase_tx.outputs.length > 0);
            auto coinbase_tx_hash = this.coinbase_tx.hashFull();
            ...
            if (coinbase_tx_hash in local_unknown_txs)
                local_unknown_txs.remove(coinbase_tx_hash);
            else // Either hash is not matching or Coinbase tx is missing
                return "Missing matching Coinbase transaction";
            tx_set ~= this.coinbase_tx;
        }
        if (local_unknown_txs.length > 0)
        {
...
public class ValidatingLedger : Ledger
{
    ...
    ***************************************************************************/
        ...
        @safe
    {
        ...
        if (next_height % this.params.PayoutPeriod == 0)   // This is a Coinbase payout block
        {
            this.coinbase_tx = this.getCoinbaseTX(next_height);
            data.tx_set ~= this.coinbase_tx.hashFull();
        }
    }
    ...
    ***************************************************************************/
        ...
        @safe
    {
        ...
        foreach (ref Hash hash, ref Transaction tx; this.pool)
        {
            ...
            {
                result.sort();
                result.length = max_txs; // Truncate
            }
        }
        return result;
    }
