...
public class Ledger
{
    ...
    ***************************************************************************/
    ...
    private void addValidatedBlock (in Block block) @safe
    {
        ...
        {
            // Clear out paid fees
            this.fee_man.clearBlockFees(block.header.height);
        }
    ...
    ***************************************************************************/
    ...
    public Transaction getCoinbaseTX (in Height height)
        nothrow @safe
    {
        assert(height >= 2 * this.params.PayoutPeriod);
        Output[] coinbase_tx_outputs;

        // pay the validator and Commons Budget for the past block fees and rewards
        auto rewards = this.rewards.calculateBlockRewards(Height(height - this.params.PayoutPeriod), 100); // TODO: calculate how many signed
        log.trace("getCoinbaseTX: rewards = {}", rewards);
        const blockFees = this.fee_man.getBlockFees(height);
        const Amount[PublicKey] validator_payouts = this.fee_man.getValidatorFees(blockFees);
        assert(payouts.length > 0);
        Amount change = 0.coins;
        auto each_validator = rewards.validator_rewards;
        if (!change.add(each_validator.div(payouts.length - 1))) // TODO: calculate share
            assert(0, "getCoinbaseTX: Failed to add change from division from rewards for validators");
        payouts.byKeyValue().filter!(kv => kv.key != this.params.CommonsBudgetAddress)
            .each!((kv)
            {
                Amount pay_validator = kv.value;
                if (!pay_validator.add(each_validator))
                    assert(0, "getCoinbaseTX: Failed to add rewards for validators");
                if (pay_validator > Amount(0))
                    coinbase_tx_outputs ~= Output(pay_validator, kv.key, OutputType.Coinbase);
            });
        Amount pay_commons = payouts[this.params.CommonsBudgetAddress];
        if (!pay_commons.add(rewards.commons_budget_rewards))
            assert(0, "getCoinbaseTX: Failed to add block rewards for the Commons Budget payout");
        if (!pay_commons.add(change))
            assert(0, "getCoinbaseTX: Failed to add change to rewards for the Commons Budget payout");
        if (pay_commons > Amount(0))
            coinbase_tx_outputs ~= Output(pay_commons, this.params.CommonsBudgetAddress, OutputType.Coinbase);

        coinbase_tx_outputs.sort;
        auto coinbase_tx = Transaction([Input(height)], coinbase_tx_outputs);
        log.trace("getCoinbaseTX: {}", coinbase_tx.prettify);
    ...
    ***************************************************************************/
    ...
    public void storeBlockFees (in Block block) @safe
    {
        this.fee_man.storeBlockFees(block, this.getValidators(block.header.height), &this.utxo_set.peekUTXO);
    }
    ...
    ***************************************************************************/
    ...
    /// return the last block height that has had fees and rewards paid in a Coinbase transaction
...
{
    // We return 1 before the first payout is made as we use this for block catchup and Genesis is never fetched
    return Height(height < 2 * payout_period ? 1 : height - (height % payout_period) - payout_period);
}

// expected is the height of last block that has had fess and rewards paid
unittest
{
    import std.range;
    import std.typecons;
...
unittest
{
    ...
    // Create blocks from height 1 to 11 (only block 10 and 15 should have a coinbase tx)
    foreach (height; 1..11)
    {
        ...
        if (height >= 2 * testPayoutPeriod && height % testPayoutPeriod == 0)
        {
            ...
            assert(cb_txs[0].outputs.map!(o => o.value)
                .reduce!((a,b) => a + b) == next_payout_total[((height - testPayoutPeriod) - 1) / testPayoutPeriod]);
