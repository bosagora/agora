                ...
                .each!((Block block)
                    {
                        ...
                        ValidatorInfo[] validators = this.getValidators(height)
                            .enumerate.filter!(en =>
                                block.header.validators[en.index]).map!(en => en.value).array;
                        auto fees = this.fee_man.getBlockFees(block.header.height);
                        auto data_fees = this.fee_man.getBlockDataFees(block.header.height);
                        Amount[] validator_fees = this.fee_man.getValidatorFees(fees, data_fees, validators);
                        ...
                        auto commons_fees = this.fee_man.getCommonsBudgetFee(fees, validator_fees);
