...
public string isInvalidReason (const ref Block block, Height prev_height,
    ...
    size_t active_enrollments, immutable(ConsensusParams) params) nothrow @safe
{
    import std.algorithm;
    ...
    {
        if (auto fail_reason = VTx.isInvalidReason(tx, findUTXO, block.header.height, params))
            return fail_reason;
    }
...
unittest
{
    ...
    assert(!block.isValid(Height(0), Hash.init, null, Enrollment.MinValidatorCount, params));
    ...
    size_t active_enrollments, immutable(ConsensusParams) params) nothrow @safe
{
    return isInvalidReason(block, prev_height, prev_hash, findUTXO,
        active_enrollments, params) is null;
}
...
unittest
{
    ...
    assert(block.isValid(GenesisBlock.header.height, gen_hash, findUTXO,
        Enrollment.MinValidatorCount, params));
    ...
    assert(block.isValid(prev_block.header.height, prev_block.header.hashFull(),
        findUTXO, Enrollment.MinValidatorCount, params));
    ...
    assert(block.isValid(GenesisBlock.header.height, GenesisBlock.header.hashFull(),
            findNonSpent, Enrollment.MinValidatorCount, params));
...
unittest
{
    ...
    // in the new block is 0, the block is considered invalid.
    {
        ...
        assert(!block3.isValid(block2.header.height, hashFull(block2.header), findUTXO, 0, params));
    }
    ...
    // then new block does not need to contain new enrollments to be considered valid
    {
        ...
        findUTXO = utxo_set.getUTXOFinder();
        assert(block3.isValid(block2.header.height, hashFull(block2.header), findUTXO, 1, params));
    }
}
