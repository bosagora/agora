...
public class EnrollmentManager
{
    ...
    ***************************************************************************/
        ...
        size_t block_height, scope UTXOFinder finder) @safe
    {
        ...
        if (this.validator_set.canAdd(block_height, finder, enroll))
        {
    ...
    /***************************************************************************
        ...
        Check if a validator can be added or can be updated it's enrolled height.
        ...
        Returns:
            If a validator can be added normally, return true,
            otherwise return false.

    ***************************************************************************/

    public bool canAddValidator (const ref Enrollment enroll,
        size_t block_height, scope UTXOFinder finder) @safe
    {
        this.enroll_pool.remove(enroll.utxo_key);

        return this.validator_set.canAdd(block_height, finder, enroll);
    }
...
unittest
{
    ...
    assert(man.canAddValidator(enroll, 9, &storage.findUTXO));
    ...
    assert(man.createEnrollment(utxo_hash, 1, enroll));
    assert(man.canAddValidator(enroll, 10, &storage.findUTXO));
    ...
    assert(man.canAddValidator(enroll2, 11, &storage.findUTXO));
    ...
    assert(man.canAddValidator(enroll3, 1019, &storage.findUTXO));
...
unittest
{
    ...
    assert(man.canAddValidator(enroll, 2, &storage.findUTXO));
