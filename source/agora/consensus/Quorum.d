...
public void verifyQuorumsSanity (in QuorumConfig[NodeID] quorums)
{
...
version (Windows)
public bool verifyQuorumsIntersect (QuorumConfig[NodeID])
{
    // @bug@: Need to fix linking issues with QuorumIntersectionChecker.create()
    return true;
}
else
public bool verifyQuorumsIntersect (QuorumConfig[NodeID] quorums, in bool split_check = true,
    string file = __FILE__, int line = __LINE__ )
{
    ...
    //     try { writefln!"verifyQuorumsIntersect: line %s:%s"(file, line); } catch (Exception) {}
    //     scope(exit)
    //     {
    //         watch.stop();
    //         writefln!"verifyQuorumsIntersect: line %s:%s (took %s msecs)"(file, line, watch.peek.total!"msecs");
    //     }
    // }
