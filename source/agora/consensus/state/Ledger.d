...
public class Ledger
{
   ...
   /***************************************************************************
        ...
        Requesting entries not in the Ledger will lead to an
    ...
    ***************************************************************************/
    ...
    public auto getBlocks (HeightRange) (HeightRange rng) @safe nothrow
    {
        const max = this.height();
        return rng.map!((Height h)
        {
            ensure(h <= max,
                "Requested height {} from getBlocks but Ledger is at {}", h, max);
            return this.storage.readBlock(h);
        });
    }
    ...
    ***************************************************************************/
    ...
    public auto getBlocksFrom (Height start) @safe nothrow
    {
        start = min(start, this.height() + 1);
        return this.getBlocks(iota(start, this.height() + 1));
    }
