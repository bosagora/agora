...
extern(C++, class) public struct BallotProtocol
{
    ...
    extern (C++, class) public struct SCPBallotWrapper
    {
    extern(C++):
      ...
      public:
        this(uint c, ValueWrapperPtr vw);
        this(ref const(SCPBallotWrapper) o);
    ...
    map!(NodeID, SCPEnvelopeWrapperPtr) mLatestEnvelopes; // M
    SCPPhase mPhase;                            // Phi
    ...
    SCPEnvelopeWrapperPtr mLastEnvelopeEmit;
  ...
  public:
    ...
    static vector!Value getStatementValues(ref SCPStatement st);
  ...
  private:
    ...
    bool attemptAcceptPrepared(const ref SCPStatement hint);
    // prepared: ballot that should be prepared
    bool setAcceptPrepared(const ref SCPBallot prepared);

    // step 2+3+8 from the SCP paper
    // ballot is the candidate to record as 'confirmed prepared'
    bool attemptConfirmPrepared(const ref SCPStatement hint);
    // newC, newH : low/high bounds prepared confirmed
    bool setConfirmPrepared(const ref SCPBallot newC, const ref SCPBallot newH);
    ...
    void recordEnvelope(SCPEnvelopeWrapperPtr);
    ...
    SCPBallotWrapperUPtr makeBallot(SCPBallot* b) const;
    SCPBallotWrapperUPtr makeBallot(uint c, Value* v) const;
}
